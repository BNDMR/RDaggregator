...
#'
#' - `"RDaggregator_gene_file"` refers to the version of the file used to associated genes to ORPHAcodes.
#'
...
#' @examples
#' \dontrun{RDaggregator_options()}
RDaggregator_options = function(){
  # Show current options
  cat('Current options:\n\n')
  cat(paste0('RDaggregator_dict: ', getOption('RDaggregator_dict'), '\n'))
  cat(paste0('RDaggregator_nomenclature: ', getOption('RDaggregator_nomenclature'), '\n'))
  cat(paste0('RDaggregator_gene_file: ', getOption('RDaggregator_gene_file'), '\n'))
  cat('\n')
  ...
  choice = readLines(con=getOption('RDaggregator.connection'), n=1) %>% as.numeric()
  cat('\n')
  ...
  if(choice == 1){
    ...
    option_type = 'RDaggregator_nomenclature'
    ...
    option_type = 'RDaggregator_gene_file'
...
default_dict = function(){
  ...
  if(is.null(getOption('RDaggregator_dict')))
    options('RDaggregator_dict'=dict)
...
default_pack_version = function(){
  ...
  if(is.null(getOption('RDaggregator_nomenclature')))
    options('RDaggregator_nomenclature'=version)
  return(version)
}
...
default_genes_version = function(){
  ...
  # Set as active if the option hasn't been set yet
  if(is.null(getOption('RDaggregator_gene_file')))
    options('RDaggregator_gene_file'=version)
